                ...
                <x-admin.ui.card class="h-100">
                    <x-admin.ui.card.body class="position-relative">
                        ...
                        <div class="flex-thumb-wrapper mb-3  align-items-center">
                            <div class="thumb">
                                <img src="{{ getImage(getFilePath('extensions') . '/' . $storage->image, getFileSize('extensions')) }}" class="thumb-img">
                            </div>
                        ...
                        <div class="btn--group">
                            <button type="button" class="flex-sm--fill btn  btn-outline--primary  editBtn" data-name="{{ __($storage->name) }}" data-shortcode="{{ json_encode($storage->shortcode) }}"
                                data-action="{{ route('admin.extensions.update', $storage->id) }}">
                                <span class=" btn--icon"><i class="la la-tools"></i></span>@lang('Configure')
                            </button>
                            <button type="button" class="flex-sm--fill btn  btn-outline--secondary helpBtn" data-description="{{ __($storage->description) }}" data-support="{{ __($storage->support) }}">
                                <span class="btn--icon"><i class="la la-info"></i></span>@lang('Help')
                            </button>
                            @if ($storage->status == Status::DISABLE)
                                <button type="button" class="flex-sm--fill btn  btn-outline--success  confirmationBtn" data-action="{{ route('admin.extensions.status', $storage->id) }}" data-question="@lang('Are you sure to enable this extension?')">

                                    <span class="btn--icon"><i class="la la-eye"></i></span>@lang('Enable')
                                </button>
                            @else
                                <button type="button" class="flex-sm--fill btn  btn-outline--danger  confirmationBtn" data-action="{{ route('admin.extensions.status', $storage->id) }}" data-question="@lang('Are you sure to disable this extension?')">
