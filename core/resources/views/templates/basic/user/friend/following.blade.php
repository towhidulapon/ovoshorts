    ...
    <div class="body__wrapper">
        <div class="body__wrapper-container">
            <div class="explore-section">
                <div class="explore-item-follower following-container">
                    @include('Template::user.friend.following_users', ['users' => $users])
                </div>
...
@push('script')
    ...
    <script>
        ...
        function initVideoHover() {
            $('.following-container .explore-item').each(function () {
                let $video = $(this).find('.video-player');

                $(this).on('mouseenter', function () {
                    if ($video.length) {
                        $video[0].muted = true;
                        $video[0].play().catch(err => console.log("Play failed:", err));
                    }
                });
        ...
        function initPlyr() {
            ...
            const players = Array.from(document.querySelectorAll('.following-container .video-player')).map(p => new Plyr(p));
        ...
        function loadMoreUsers() {
            ...
            $.ajax({
                url: "{{ route('user.friend.following.list') }}?page=" + currentPage,
                type: 'GET',
                success: function (response) {
                    if (response.status == "success") {
                        $('.following-container').append(response.data.data);
                        currentPage++;
        ...
        $(document).on("click", ".follow-btn", function (e) {
            ...
            $.ajax({
                url: "{{ url('user/friend/unfollow') }}/" + userId,
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                },
                success: function (response) {
                    if (response.status === "success") {
                         $btn.closest(".explore-item").fadeOut( function () {
                            $(this).remove();
