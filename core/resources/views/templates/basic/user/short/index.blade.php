...
@section('content')
    ...
    <div class="dashboard-body upload-step1 {{ $short || $latestDraft ? 'd-none' : '' }}">
            ...
            <div class="video-upload-card__wrapper">
                ...
                <div class="video-upload-card__content text-center video-drop-area">
                    ...
                    <div class="avatar-edit seller-cover-photo">
                        <input class="profilePicUpload video-input" id="profilePicUpload1" name="video" accept="video/*" type="file" required>
    ...
    <div class="dashboard-body upload-step2  {{ ($short || $latestDraft) ? '' : 'd-none' }}" data-latest-short-id="{{ $latestDraft->id ?? '' }}" data-video-path="{{ $latestDraft->name ?? '' }}" data-storage-driver="{{ $latestDraft->storage_driver ?? '' }}" data-filename="{{ $latestDraft->name ?? '' }}">
        <div class="dashboard-body__bar d-lg-none d-block">
            <span class="dashboard-body__bar-icon"><i class="fas fa-bars"></i></span>
        </div>
        @if (!$short)
            <div class="video-upload__item">
                <div class="video-upload__item__inner">
                    <div class="video-upload__content">
                        <div class="d-flex gap-2 align-items-start">
                            <div class="">
                                <span class="video-upload__filename filename"></span>
                ...
                <div class="video-upload__progress">
                    <div class="video-upload__progress-bar progress-bar" style="width: 0%;"></div>
                </div>
            </div>
        @endif

        <div class="row mt-4">
            <form action="{{ route('user.short.upload.store', $short->id ?? null) }}" enctype="multipart/form-data" method="POST">
                @csrf
                <input type="hidden" name="short_id" class="short-id" value="{{ $latestDraft->id ?? '' }}">
                <input type="hidden" name="cover_image_data" class="cover-image-data">
                <div class="video-upload-edit">
                                        ...
                                        <div class="tab-pane fade show active" id="pills-home" role="tabpanel" aria-labelledby="pills-home-tab" tabindex="0">
                                            <div class="explore-item">
                                                <div class="explore-item__video">
                                                    <video class="video-player" playsinline preload="metadata" data-video_id="{{ $latestDraft->id ?? '' }}" controls poster="{{ isset($latestDraft->cover_image) ? getImage(getFilePath('coverImage') . '/' . $latestDraft->cover_image) : asset('assets/images/thumbs/edit-preview.png') }}">
                                                        <source src="{{ isset($latestDraft->temp_path) && Storage::disk('local')->exists($latestDraft->temp_path) ? getImage(getFilePath('shortFile') . '/' . $latestDraft->upload_id) : ''}}" class="video-source" type="video/mp4">
                                                    </video>
                                                </div>
                                            </div>
                                        </div>
                    ...
                    <div class="video-upload-edit__details">
                        <div class="video-upload-edit__item mb-4">
                            <h4 class="title-heading">@lang('Details')</h4>
                            <div class="video-upload-edit__item-card">
                                <h6 class="title">@lang('Description')</h6>
                                <div class="video-upload-form">
                                    <textarea class="form-control form--control video-description" name="description" placeholder="Description">{{ old('description', $short->description ?? '') }}</textarea>
                                    <div class="video-upload-tags">
                                        <div class="video-upload-tags__left">
                                            <button class="btn video-upload-tags__button hash-btn">#Hashtags</button>
                                        </div>
                                ...
                                @if (!$short)
                                    <div class="video-edit-cover">
                                        ...
                                        <div class="video-edit-cover__thumb">
                                            <img class="fit-image cover-image" src="{{ isset($latestDraft->cover_image) && $latestDraft->cover_image ? getImage(getFilePath('coverImage') . '/' . $latestDraft->cover_image) : asset($activeTemplateTrue . 'images/default.png') }}" alt="image">
                                            <input type="file" class="cover-upload d-none" name="cover_image" accept="image/*">
                                            <button class="video-edit-cover__edit cover-edit-btn">
                                                @lang('Edit cover')
                                            </button>
                                        </div>
                        ...
                        <div class="video-upload-edit__item">
                            <h4 class="title-heading">@lang('Settings')</h4>
                            <div class="video-upload-edit__item-card">
                                @if ($short === null)
                                    <h6 class="title">@lang('When to post')</h6>
                                    <div class="post-schedule">
                                        <div class="form-check form--radio">
                                            <input class="form-check-input post-now" type="radio" name="post_at" value="1" id="radioDefault1" checked>
                                            <label class="form-check-label" for="radioDefault1">
                                                @lang('Now')
                                            </label>
                                        </div>
                                        <div class="form-check form--radio">
                                            <input class="form-check-input post-schedule-radio" type="radio" name="post_at" value="2" id="radioDefault2">
                                ...
                                <div class="video-edit-user mb-0">
                                    ...
                                    <div class="video-edit-user__check">
                                        <div class="form--check">
                                            <input class="form-check-input allow-comment" name="comment" type="checkbox" value="1" id="comment" {{ old('comment', $short?->allow_comments ?? 1) ? 'checked' : '' }}>
                                            <label class="form-check-label" for="comment">@lang('Comment')</label>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="video-edit-btn-group mt-4 flex-wrap gap-2">
                                <button class="btn btn--base btn-post" type="submit">@lang('Post')</button>
                                <button class="btn btn-dark discard">@lang('Discard')</button>
                            </div>
                        </div>
                    </div>
                </div>
...
@push('style')
    <style>
        .video-upload-card__wrapper.drag-hover {
...
@push('script')
    <script>
        (function ($) {
            ...
            const dropArea = document.querySelector('.video-upload-card__wrapper');
            ...
            if ($('.upload-step2').data('latest-short-id')) {
                ...
                const latestVideoPath = $('.upload-step2').data('video-path');
                const latestStorageDriver = $('.upload-step2').data('storage-driver');
                ...
                const videoUrl = '{{ asset(getFilePath('shortFile')) }}/' + latestVideoPath;
            ...
            function proceedWithUpload() {
                ...
                $('.upload-step2').removeClass('d-none');
                $('.filename').text(videoFile.name);
                $('.upload-status').text('Chunking...');
                $('.progress-label').text('Chunking Progress');
            ...
            function initiateUpload() {
                $.ajax({
                    type: "POST",
                    url: "{{ route('user.short.upload.initiate') }}",
                    data: {
                    ...
                    success: function (response) {
                        if (response.data && response.data.success) {
                            ...
                            notify('error', 'Failed to initiate upload');
                            resetUpload();
                        }
                    }
                });
            ...
            function uploadNextChunk() {
                ...
                $.ajax({
                    type: "POST",
                    url: '{{ route('user.short.upload.chunk') }}',
                    data: formData,
                    processData: false,
                    contentType: false,
                    headers: {
                    ...
                    success: function (response) {
                        if (response.data && response.data.success) {
                            currentChunk++;
                            const progress = (currentChunk / chunkCount) * 100;
                            $('.progress-bar').css('width', progress + '%');
                            uploadNextChunk();
                        } else {
                            notify('error', 'Failed to upload chunk');
                            resetUpload();
                        }
                    }
                });
            ...
            function completeChunking() {
                $('.upload-status').text('Uploading to Server...');
                $('.progress-label').text('Uploading Progress');
                $('.progress-bar').css('width', '0%');
                ...
                $.ajax({
                    type: "POST",
                    url: '{{ route('user.short.upload.complete') }}',
                    data: {
                        upload_id: uploadId,
                        filename: videoFile.name
                    },
                    ...
                    success: function (response) {
                        ...
                        if (response.data && response.data.success) {
                            ...
                            setCoverImage(videoFile);
                            setTimeout(function () {
                                location.reload();
                            }, 500);
                        } else {
                            notify('error', 'Failed to complete upload');
                            resetUpload();
                        }
                    }
                });
            ...
            function resetUpload() {
                $('.upload-step1').removeClass('d-none');
                $('.upload-step2').addClass('d-none');
                $('.video-input').val('');
                $('.progress-bar').css('width', '0%');
                $('.progress-label').text('Upload in Progress');
                $('.filename').text('');
                $('.upload-status').text('Uploading...');
                $('.cover-image').attr('src', '');
                $('.cover-image-data').val('');
                $('.cover-upload').val('');
                videoFile = null;
                uploadId = null;
                chunkCount = 0;
            ...
            function saveCoverImageToDraft(shortId, dataUrl) {
                ...
                $.ajax({
                    type: "POST",
                    url: '{{ route('user.short.upload.draft') }}',
                    data: {
                        short_id: shortId,
                    ...
                    success: function (response) {
                        if (response.data && response.data.success) {
                            console.log('Cover image saved to draft.');
                        } else {
                            notify('error', 'Failed to save cover image to draft');
                        }
                    }
                });
            }
            ...
            function setCoverImage(file) {
                ...
                video.addEventListener('loadeddata', function () {
                    ...
                    video.addEventListener('seeked', function () {
                        ...
                        URL.revokeObjectURL(video.src);
                        saveCoverImageToDraft($('.short-id').val(), dataUrl);
                    });
                });
            }

            function setCoverImageFromUrl(url) {
                const video = document.createElement('video');
                video.src = url;
                video.addEventListener('loadeddata', function () {
                    video.currentTime = 1;
            ...
            $('.cover-edit-btn').on('click', function (e) {
                e.preventDefault();
            ...
            $('.cover-upload').on('change', function (e) {
                ...
                if (coverFile) {
                    ...
                    $('.cover-image').attr('src', coverUrl);
                    const reader = new FileReader();
                    reader.onload = function (e) {
                        $('.cover-image-data').val(e.target.result);
                    };
                    reader.readAsDataURL(coverFile);
                    console.log('Custom cover image selected: ' + coverFile.name);
                }
            });

            $('.hash-btn').on('click', function (e) {
                e.preventDefault();
                let descriptionField = $('.video-description');
            ...
            $('.video-description').on('input', function () {
                ...
                $('.char-count').text(`${count}/4000`);
                if (count > 4000) {
                    $(this).val($(this).val().substring(0, 4000));
                    $('.char-count').text('4000/4000');
                }
            });

            $('.post-schedule-radio').on('change', function () {
                if ($(this).is(':checked')) {
                    $('.schedule-datetime').removeClass('d-none');
                }
            });

            $('.post-now').on('change', function () {
                if ($(this).is(':checked')) {
                    $('.schedule-datetime').addClass('d-none');
                }
            });
