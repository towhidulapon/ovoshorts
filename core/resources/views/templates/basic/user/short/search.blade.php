                ...
                <div class="explore-item-wrapper">
                    @forelse ($shorts as $userShort)
                        <div class="explore-item">
                            <a href="{{ $userShort->is_placeholder ? '#' : route('user.short.view', $userShort->id) }}" class="explore-item__link">
                                <div class="explore-item__video">
                                    <video class="video-player" playsinline preload="metadata" controls poster="{{ $userShort->is_placeholder ? asset($activeTemplateTrue . 'images/default.png') : getImage(getFilePath('coverImage') . '/' . $userShort->cover_image, getFileSize('coverImage')) }}">
                                        @unless($userShort->is_placeholder)
                                            <source src="{{ $userShort->fileUrl }}" type="video/{{ $userShort->extension }}">
                                        @endunless
                                    </video>

                                    @unless($userShort->is_placeholder)
                                        <button class="explore-action explore-action__button like-btn {{ $userShort->isLiked ? 'liked' : '' }}" data-shorts-id="{{ $userShort->id }}" data-shorts-owner-id="{{ $userShort->user_id }}">
                                            <span class="icon">
