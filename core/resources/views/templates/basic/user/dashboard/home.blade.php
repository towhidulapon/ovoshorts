...
<div class="video-studio__profile mb-4">
    ...
    <div class="video-studio__profile__content">
        <div class="name"><span class="name__title">{{ auth()?->user()?->username}}</span> | <span class="name__subtitle">{{ auth()?->user()?->bio }}</span></div>
        <ul class="video-studio__profile__social">
            <li class="item"><span>@lang('Likes')</span> <span>{{ showFormatCount(auth()?->user()?->totalLikes) }}</span></li>
            <li class="item"><span>@lang('Followers')</span> <span>{{ showFormatCount(auth()?->user()?->followers?->count()) }}</span></li>
            <li class="item"><span>@lang('Following')</span> <span>{{ showFormatCount(auth()?->user()?->followings?->count()) }}</span></li>
        </ul>
    </div>
</div>
    ...
    <div class="dashboard-card-wrapper">
        <div class="dashboard-widget">
            <div class="dashboard-widget__content">
                ...
                <h4 class="dashboard-widget__number">{{ showFormatCount(auth()?->user()?->total_views) }}</h4>
            </div>
        </div>

        <div class="dashboard-widget">
            <div class="dashboard-widget__content">
                <span class="dashboard-widget__text">@lang('Likes')</span>
                <h4 class="dashboard-widget__number">{{ showFormatCount(auth()?->user()?->totalLikes) }}</h4>
            </div>
        </div>
        <div class="dashboard-widget">
            <div class="dashboard-widget__content">
                <span class="dashboard-widget__text">@lang('Comments')</span>
                <h4 class="dashboard-widget__number">{{ showFormatCount(auth()?->user()?->totalComments) }}</h4>
            </div>
        </div>
        <div class="dashboard-widget">
            <div class="dashboard-widget__content">
                <span class="dashboard-widget__text">@lang('Shares')</span>
                <h4 class="dashboard-widget__number">{{ showFormatCount(auth()?->user()?->totalShares) }}</h4>
            </div>
        </div>
    </div>
</div>
@if (request()->routeIs('user.dashboard.analytics'))
    <div class="row">
        <div class="col-md-6 mb-4">
            <div class="card custom--card">
                <div class="card-header">
                    <h5 class="card-header__title mb-0 text-white">@lang('Traffic Source') <span data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Tooltip on top">
...
@if (!request()->routeIs('user.dashboard.analytics'))
    <div class="dashboard-card">
        <div class="dashboard-card__header">
            <a href="{{ route('user.dashboard.post') }}" class="link-btn">@lang('Recent Posts') <i class="las la-angle-right"></i></a>
        </div>
        <div class="dashboard-card-wrapper">
            <div>
