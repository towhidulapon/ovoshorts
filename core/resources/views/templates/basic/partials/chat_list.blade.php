<div class="message-group">
    @foreach ($chatUsers as $user)
        ...
        $lastMessage = \App\Models\Message::where(function ($q) use ($user) {
            ...
            ->first();
                @endphp
                <div class="message-item {{ $activeUser && $user->id == $activeUser->id ? 'active' : '' }}" data-user-id="{{ $user->id }}" data-username="{{ $user->username }}" data-firstname="{{ $user->firstname }}" data-lastname="{{ $user->lastname }}" data-image="{{ getImage(getFilePath('userProfile') . '/' . $user->image) }}">
                    <div class="message-item__left">
                        <div class="message-item__thumb thumb">
                            <img src="{{ getImage(getFilePath('userProfile') . '/' . $user->image) }}" alt="img" class="fit-image">
                            @if($user->is_online)
