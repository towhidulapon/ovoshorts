...
class ManageUsersController extends Controller
{
    ...
    public function verifiedUsers()
    {
        ...
        extract($this->userData("verified"));
    ...
    public function verificationPendingUsers()
    {
        $pageTitle = 'Verification Pending Users';
        extract($this->userData('verificationPending'));
    ...
    public function verificationDetails($id)
    {
        ...
        $user      = User::findOrFail($id);
        return view('admin.users.verification_detail', compact('pageTitle', 'user'));
    }
    ...
    public function verificationApprove($id)
    {
        ...
        notify($user, 'VERIFICATION_APPROVE', []);

        $notify[] = ['success', 'Verification approved successfully'];
        return to_route('admin.users.verification.pending')->withNotify($notify);
    }

    public function verificationReject(Request $request, $id)
    {
        $request->validate([
            'reason' => 'required'
        ]);
        ...
        $user                       = User::findOrFail($id);
        $user->is_verified                   = Status::VERIFICATION_UNVERIFIED;
        $user->verification_rejection_reason = $request->reason;
        $user->save();

        notify($user, 'VERIFICATION_REJECT', [
